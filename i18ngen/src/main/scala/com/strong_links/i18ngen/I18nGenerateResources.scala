...
class Generator(runConfig: RunConfig, i18nConfig: I18nConfig, i18nConfigLocalization: I18nConfigLocalization) extends Logging {

  val poFile = runConfig.getPoFile(i18nConfig, i18nConfigLocalization)

  val resourceFile = runConfig.getResourceFile(i18nConfig, i18nConfigLocalization)
  ...
  def run = Errors.trap(poFile) {
    ...
    new ResourceFileWriter(resourceFile, i18nConfig, i18nConfigLocalization, parseResults.headerInfo.nPlural,
...
object I18nGenerateResources {
  ...
  def run(runConfig: RunConfig) =
    runConfig.i18nConfigs.par.flatMap(c => { c.allLocalizations.par.map(new Generator(runConfig, c, _).run) }).toList
}
