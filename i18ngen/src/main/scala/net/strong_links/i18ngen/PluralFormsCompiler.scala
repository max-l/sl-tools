...
class PluralFormsCompiler(data: String) extends LexParser(data) with Logging {
  ...
  def compile = Errors.trap("Invalid plural forms expression _" << data) {
    ...
    expression
    if (symbol != eof)
      Errors.fatal("Expected end of expression.")
    emitReturn
    output += "}"
    ...
    symbol match {
      ...
      case _ if (symbol in Set(plus, minus, logicalNot)) =>
        ...
        if (unaryOp == minus)
          ...
          op(unaryOp)
      case _ =>
        Errors.fatal("Invalid symbol '_' to start a factor." << symbol)
    }
  }
