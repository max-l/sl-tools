...
class I18nGenMerge(runConfig: RunConfig) extends I18nGen(runConfig) {
  ...
  class Merger(i18nLocalization: I18nLocalization, scalaI18nCallSummaries: List[ScalaI18nCallSummary]) extends LocalizationRunner(i18nLocalization) {
  ...
  def merge = {
    ...
    runConfig.masterLocalizations.par.foreach(new Merger(_, i18nCallsInScalaFiles).run)
  }
}
